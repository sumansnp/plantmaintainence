...
<title>Preventive Maintenance - Add Maintenance Schedule</title>
...
<div class="container">
  ...
  <sql:query var="eCodeList" dataSource="jdbc/plantmaintenance">
          SELECT eCode FROM equipmentlist where eCode not in (select eCode from maintenanceschedule)
  </sql:query>
  <c:if test="${pageContext.request.method=='POST'}">
                    <c:catch var="exception">
                        <sql:update var="addMaintenanceSchedule" dataSource="jdbc/plantmaintenance">
                            insert into maintenanceSchedule(`eCode`,`dueDate`) 
                            values(?,?,?)
                            <sql:param value="${param.e_code}" />
                            <sql:param value="${param.valid_date}" />
                                               
                        </sql:update>
                        <c:if test="${addMaintenanceSchedule>=1}">
