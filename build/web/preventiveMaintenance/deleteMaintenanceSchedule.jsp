...
<title>Preventive Maintenance - Delete Maintenance Schedule</title>
<script type=text/javascript>
	...
	function validate()
	{
		...
		if(ecode=="")
		{
			window.alert("please enter schedule no");
			document.form1.e_code.focus();
			return false;
		}
		else if(isNaN(ecode))
		{
			window.alert("invalid schedule no");
			document.form1.e_code.focus();
			return false;
		}
                else
...
<div class="container">
  ...
  <div class="content">
  <h1>Delete Maintenance Schedule</h1>
<c:if test="${pageContext.request.method=='POST'}">
                    <c:catch var="exception">
                        <sql:update var="deleteMaintenanceSchedule" dataSource="jdbc/plantmaintenance">
                            DELETE FROM maintenanceSchedule
                            WHERE sno=?
                            <sql:param value="${param.e_code}"/>
                        </sql:update>
                            <c:choose>
                                <c:when test="${deleteMaintenanceSchedule>=1}">
 ...
 <form method="post" action="" name="form1" onSubmit="return validate();">
   <table width="80%" border="1" align="center">
     <tr>
       <td>Enter Maintenance Schedule No</td>
