...
<title>Calibration Tracking - Delete Calibration Schedule</title>
<script type=text/javascript>
	...
	function validate()
	{
		...
		if(icode=="")
		{
			window.alert("please enter calibration schedule no");
			document.form1.i_code.focus();
			return false;
		}
		else if(isNaN(icode))
		{
			window.alert("invalid calibration schedule no");
			document.form1.i_code.focus();
			return false;
		}
                else
...
<li><a href="viewRepairerSummaryReport.jsp">View Repairer Details</a></li>
  ...
  <h1>Delete Calibration Schedule</h1>
<c:if test="${pageContext.request.method=='POST'}">
                    <c:catch var="exception">
                        <sql:update var="deleteCalibrationSchedule" dataSource="jdbc/plantmaintenance">
                            DELETE FROM calibrationSchedule
                            WHERE csNo=?
                            <sql:param value="${param.i_code}"/>
                        </sql:update>
                            <c:choose>
                                <c:when test="${deleteCalibrationSchedule>=1}">
 ...
 <form method="post" action="" name="form1" onSubmit="return validate();">
   <table width="80%" border="1" align="center">
     <tr>
       <td>Enter Calibration Schedule No</td>
       <td><input type="text" name="i_code" id="icode" /></td>
     </tr>
     <tr>
       <td>&nbsp;</td>
       <td align="right"><input type="submit" name="btn_view" id="btn_view" value="Delete" />
