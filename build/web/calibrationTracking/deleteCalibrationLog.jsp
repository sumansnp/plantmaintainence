...
<title>Calibration Tracking - Delete Calibration Log</title>
<script type=text/javascript>
	...
	function validate()
	{
		...
		if(icode=="")
		{
			window.alert("please enter calibration log no");
			document.form1.i_code.focus();
			return false;
		}
		else if(isNaN(icode))
		{
			window.alert("invalid calibration log no");
			document.form1.i_code.focus();
			return false;
		}
		else
...
<li><a href="viewRepairerSummaryReport.jsp">View Repairer Details</a></li>
   ...
   <div class="content">
  <h1>Delete Calibration Log</h1>
<c:if test="${pageContext.request.method=='POST'}">
                    <c:catch var="exception">
                        <sql:update var="deleteCalibrationLog" dataSource="jdbc/plantmaintenance">
                            DELETE FROM calibrationLog
                            WHERE clNo=?
                            <sql:param value="${param.i_code}"/>
                        </sql:update>
                            <c:choose>
                                <c:when test="${deleteCalibrationLog>=1}">
 ...
 <form method="post" action="" name="form1" onSubmit="return validate();">
   <table width="80%" border="1" align="center">
     <tr>
       <td>Enter Calibration Log No</td>
