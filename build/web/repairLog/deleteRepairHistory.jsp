...
<html>
<head>
<title>Repair Log - Delete Repair History</title>
...
<script type="text/javascript">
	...
	function validate()
	{
		...
		if(icode=="")
		{
			window.alert("please enter repair history no");
			document.form1.i_code.focus();
			return false;
		}
		else if(isNaN(icode))
		{
			window.alert("invalid repair history no");
			document.form1.i_code.focus();
			return false;
		}
...
<li><a href="viewCalibrationLogSummaryReport.jsp">View Calibration Log</a></li>
   ...
   <div class="content">
    <h1>Delete Repair History</h1>
    <p>&nbsp;</p>
    <c:if test="${pageContext.request.method=='POST'}">
                    <c:catch var="exception">
                        <sql:update var="deleteRepairHistory" dataSource="jdbc/plantmaintenance">
                            DELETE FROM repairHistory
                            WHERE rhNo=?
                            <sql:param value="${param.i_code}"/>
                        </sql:update>
                            <c:choose>
                                <c:when test="${deleteRepairHistory>=1}">
    ...
    <form action="" method="post" name="form1" onSubmit="return validate();">
    	<table border=1 align=center width=80%>
        	<tr>
            	<td width="50%">Enter Repair History No<td width="50%"><input type="text" name="i_code">
